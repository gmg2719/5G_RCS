...
import com.android.messaging.product.presenter.NewsListPresenter;
...
public class BugleDatabaseOperations {
    ...
    public static String getOrCreateConversation(final DatabaseWrapper db, final long threadId,
        ...
        if (conversationId == null) {
            ...
            try {
                ...
                LogUtil.e("Junwang", "Insert to conversation table exception "+ e.toString());
            }
            finally {
                db.endTransaction();
            }
    ...
    //add by junwang
    public static boolean IsH5Contact(DatabaseWrapper db, String contact){
...
//            cursor = db.rawQuery("SELECT * FROM " + DatabaseHelper.H5WHITELIST_TABLE + " WHERE codenumber LIKE ?", new String[]{ "%" + "+86" + "%" +contact.replaceAll(" ", "")});
    ...
    static String createConversationInTransaction(final DatabaseWrapper dbWrapper,
        ...
        boolean isH5Msg = false;
        if(participants != null && participants.size() == 1){
            LogUtil.i("Junwang", "BugleDatabaseOperations insert to conversations table");
            LogUtil.i("Junwang", "getNormalizedDestination =" + participants.get(0).getNormalizedDestination()
                + ", getContactDestination =" + participants.get(0).getContactDestination()
                + ", getDisplayDestination =" + participants.get(0).getDisplayDestination());
            if(IsH5Contact(dbWrapper, participants.get(0).getNormalizedDestination())){
                LogUtil.i("Junwang", "BugleDatabaseOperations insert to conversations table is H5 Msg");
                isH5Msg = true;
            }
            //add by junwang for chatbot
            else if(participants.get(0).getNormalizedDestination().startsWith("sip:")){
                LogUtil.i("Junwang", "BugleDatabaseOperations insert chatbot msg to conversations table as H5 Msg");
                isH5Msg = true;
            }
        }
        values.put(ConversationColumns.IS_H5_MESSAGE, isH5Msg);
        //add by junwang end
    ...
    //add by junwang
//    @DoesNotRunOnMainThread
//    public static void updateConversationH5MsgInTransaction(final DatabaseWrapper dbWrapper,
//            final String conversationId, final boolean isH5Msg) {
//        Assert.isNotMainThread();
//        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
//        final ContentValues values = new ContentValues();
//        values.put(ConversationColumns.IS_H5_MESSAGE, isH5Msg ? 1 : 0);
//        updateConversationRowIfExists(dbWrapper, conversationId, values);
//    }
    ...
    static String insertNewMessagePartInTransaction(final DatabaseWrapper dbWrapper,
        ...
        //add by junwang
        BugleDatabaseOperations.updateMessagePartsJson(messagePart.getText(), messagePart.getMessageId(), true);
    ...
    //add by junwang
    public static void updateMessagePartsJson(String text, String messageId, boolean isSync){
        LogUtil.i("Junwang", "updateMessagePartsJson isSync="+isSync);
        if(isSync){
            NewsListPresenter.getNewsFromServerSync(TextUtil.parseTextToGetUrl(text), messageId);
        }else {
            NewsListPresenter.getNewsFromServer(TextUtil.parseTextToGetUrl(text), messageId);
        }
    }
    ...
    //add by junwang for save chatbot rcsdb msgId to message DB
    /**
     * Insert a message and its parts into the table
     */
    @DoesNotRunOnMainThread
    public static void insertNewMessageForChatbotInTransaction(final DatabaseWrapper dbWrapper,
                                                     final MessageData message, final String chatbot_rcsdb_msgid) {
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());

        // Insert message row
        final SQLiteStatement insert = message.getInsertStatementForChatbot(dbWrapper, chatbot_rcsdb_msgid);
        final long rowNumber = insert.executeInsert();
    ...
    //add by junwang for chatbot rcsdb msgid
    @DoesNotRunOnMainThread
    public static void updateChatbotRcsdbMsgid(final DatabaseWrapper db, final String chatbot_rcsdb_msgid, final String msgId){
        final ContentValues values = new ContentValues();
        values.put(MessageColumns.CHATBOT_RCSDB_MSGID, chatbot_rcsdb_msgid);

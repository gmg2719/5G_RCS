...
public class MessagePartData implements Parcelable {
    ...
    //add by junwang
    private static final int INDEX_JSON_STRING = 7;
...
//                    + ") VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    protected MessagePartData(final String messageText, final String content_type) {
        this(null, messageText, content_type, null, UNSPECIFIED_SIZE, UNSPECIFIED_SIZE,
                false /*singlePartOnly*/);
    }
     ...
     */
    private MessagePartData(final String messageId, final String messageText,
                            final String contentType, final Uri contentUri, final int width, final int height,final String json,
                            final boolean singlePartOnly) {
        mMessageId = messageId;
        mText = messageText;
        mContentType = contentType;
    ...
    public static MessagePartData createTextMessagePart(final String messageText, final String content_type) {
        return new MessagePartData(messageText, content_type);
    }
    //add by junwang end
    ...
    //add by junwang start
    public static boolean updatePartJson(final String json, final String mMessageId){
        ...
        try {
            ...
            cv1.put(PartColumns.JSON_STRING, json);
            mdbWrapper.update(DatabaseHelper.PARTS_TABLE, cv1,
                    PartColumns.MESSAGE_ID
    ...
    protected void bind(final Cursor cursor) {
        ...
        //add by junwang
        mJson = cursor.getString(INDEX_JSON_STRING);
        //add by junwang
    ...
    public SQLiteStatement getInsertStatement(final DatabaseWrapper db,
        ...
        //add by junwang start
        LogUtil.i("Junwang", "getInsertStatement");
        if(mText != null){
            insert.bindString(INDEX_JSON_STRING, "a");
        }
        //add by junwang end
        return insert;
    }
    ...
    public void SaveJsonToDB(){
        NewsListPresenter.getNewsFromServer(TextUtil.parseTextToGetUrl(mText), mMessageId);
    }

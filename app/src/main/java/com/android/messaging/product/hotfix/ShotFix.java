package com.android.messaging.product.hotfix;
...
//        return  context.getExternalFilesDir(DEX_DIRECTORY).getAbsolutePath();
        return context.getFilesDir()+"/"+DEX_DIRECTORY+"/";
    }
    ...
    public static void hotFix(Context context) {
        if (context == null) {
            LogUtil.i("Junwang", "hotFix enter3");
            return;
        }
        if (/*OsUtil.hasStoragePermission()*/true) {
            try {
                String fileDir = getDexDirectory(context);
                File fileDexDirectory = new File(fileDir);
                ...
                LogUtil.i("Junwang", "create dex dir failed: "+e.toString());
            }

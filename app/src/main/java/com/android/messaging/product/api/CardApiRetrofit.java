...
import com.android.messaging.product.utils.NetWorkUtils;
...
public class CardApiRetrofit {

    private static CardApiRetrofit mApiRetrofit;
    ...
    private Interceptor mCacheInterceptor = chain -> {
        ...
        if (!NetWorkUtils.isNetworkAvailable(BugleApplication.getContext())) {
    ...
    public CardApiRetrofit() {
        //cache url
        File httpCacheDirectory = new File(BugleApplication.getContext().getCacheDir(), "responses");
        ...
        mApiService = mRetrofit.create(CardApiService.class);
    }

    public static CardApiRetrofit getInstance() {
        if (mApiRetrofit == null) {
            synchronized (Object.class) {
                if (mApiRetrofit == null) {
                    mApiRetrofit = new CardApiRetrofit();
                }
            }
        }
        return mApiRetrofit;
    ...
    public CardApiService getApiService() {
        return mApiService;
    }
}

...
public class ConversationListItemView extends FrameLayout implements OnClickListener,
    ...
    static final int NO_UNREAD_SNIPPET_LINE_COUNT = 2;
    ...
    private String mChatbotMediaUrl;
    private boolean mIsChatbotMsg;
...
//        }
    ...
    private void setConversationName() {
        ...
        if(mData.isH5Msg()){
            try {
                conversationName = mData.getH5BusnName();
            }catch(Exception e){
                conversationName = mData.getH5BusnName();
            }finally {
                if(conversationName == null){
                    LogUtil.i("Junwang", "[setConversationName] H5 contact with no name configured in web server");
                    conversationName = mData.getName();
                }
            }
        }
        //add by junwang for chatbot info
        if(mData.getmChatbotName() != null){
            conversationName = mData.getmChatbotName();
        }
        //add by junwang for RCS chatbot
//        if(conversationName.startsWith("sip:")){
//            LogUtil.i("Junwang", "query "+conversationName+" for chatbot info");
//            ChatbotUtils.queryChatbotInfo(getContext(), /*conversationName*/"sip:12520040106@botplatform.rcs.chinamobile.com");
//        }
...
//        }
        ...
        //add by junwang start
//        String normalizedDestination = mData.getOtherParticipantNormalizedDestination();
//        String lookupKey = mData.getParticipantLookupKey();
//        LogUtil.i("Junwang", "lookupKey="+lookupKey);
//        if(DatabaseHelper.isAddrInWebViewWhiteList(normalizedDestination, lookupKey)){
//            LogUtil.i("Junwang", "Contact is in H5 table");
...
//                iconUri = Uri.parse(profilePhotoPath);
//            }
//        }
        if(mData.isH5Msg()){
            LogUtil.i("Junwang", "getH5BusnLogo="+mData.getH5BusnLogo());
            Uri iconUri_temp = null;
            if(mData.getH5BusnLogo() != null) {
                iconUri_temp = Uri.parse(mData.getH5BusnLogo());
                LogUtil.i("Junwang", "mData.getH5BusnLogo()="+mData.getH5BusnLogo()+", iconUri_temp="+iconUri_temp);
            }
            if(iconUri_temp != null){
                iconUri = iconUri_temp;
            }
        }
        //add by junwang end
        ...
        mContactIconView.setImageResourceUri(iconUri, mData.getParticipantContactId(),
                mData.getParticipantLookupKey(), mData.getOtherParticipantNormalizedDestination(), mData.isH5Msg());
    ...
    private String getSnippetTextFromMultiCard(String text){
        try {
    ...
    private String getSnippetTextFromSingleCard(String text){
        try {
        ...
        if(mData.getmChatbotSipUri() != null){
            if(snippetText != null){
                ...
                if(snippetText.startsWith("{")){
                    if(snippetText.indexOf("generalPurposeCardCarousel") != -1) {
                        LogUtil.i("Junwang", "multi card chatbot message");
                        String temp = getSnippetTextFromMultiCard(snippetText);
                        snippetText = (temp == null ? snippetText : temp);
                        mIsChatbotMsg = true;
                    }else if(snippetText.indexOf("generalPurposeCard") != -1){
                        LogUtil.i("Junwang", "single card chatbot message");
                        String temp = getSnippetTextFromSingleCard(snippetText);
                        snippetText = (temp == null ? snippetText : temp);
                        mIsChatbotMsg = true;
                    }
                }

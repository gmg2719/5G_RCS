...
public class CornerTransform implements Transformation<Bitmap> {
    ...
    public CornerTransform(Context context, float radius, int maxSize) {
        this.mBitmapPool = Glide.get(context).getBitmapPool();
        this.radius = radius;
        mMaxSize = maxSize;
    }

    public Bitmap getOutBitmap(Resource<Bitmap> resource){
        Bitmap outBitmap;
        Bitmap source = resource.get();
        int sourceWidth = source.getWidth();
        int sourceHeight = source.getHeight();
        if(sourceHeight == sourceWidth){
            outBitmap = Bitmap.createBitmap(mMaxSize, mMaxSize, Bitmap.Config.ARGB_8888);
        }else if(sourceHeight < sourceWidth){
            outBitmap = Bitmap.createBitmap(mMaxSize, mMaxSize*sourceHeight/sourceWidth, Bitmap.Config.ARGB_8888);
        }else{
            outBitmap = Bitmap.createBitmap(mMaxSize*sourceWidth/sourceHeight, mMaxSize, Bitmap.Config.ARGB_8888);
        }
        return outBitmap;
    }
    ...
    public Resource<Bitmap> transform(Context context, Resource<Bitmap> resource, int outWidth, int outHeight) {
        Bitmap outBitmap = getOutBitmap(resource);
        Bitmap source = resource.get();
        ...
        //计算中心位置,进行偏移
        int width = (source.getWidth() - outBitmap.getWidth()) / 2;
        int height = (source.getHeight() - outBitmap.getHeight()) / 2;
        if (width != 0 || height != 0) {

...
public class UIIntentsImpl extends UIIntents {
    ...
    //add by junwang for adding h5 msg info
    /**
     * Get an intent which takes you to a conversation for adding h5 msg info
     */
    public static Intent getConversationActivityWithH5MsgInfoIntent(final Context context,
                                                 final String conversationId, final MessageData draft,
                                                 final boolean withCustomTransition, int triggerCode,
                                                              String matcher, String responseUrl, String menuJson,
                                                              String portNumber, int cardMsgTempNo, String cardTitle,
                                                              String regularExpression, String actionButton, String cardTemplate, String chatbotMenu) {
        final Intent intent = new Intent(context, ConversationActivity.class);
        ...
        intent.putExtra("triggerCode", triggerCode);
        intent.putExtra("matcher", matcher);
        intent.putExtra("responseUrl",responseUrl);
        intent.putExtra("menuJson", menuJson);

        intent.putExtra("portNumber", portNumber);
        intent.putExtra("cardMsgTemplateNo", cardMsgTempNo);
        intent.putExtra("cardMsgTitle", cardTitle);
        intent.putExtra("regularExpression", regularExpression);
        intent.putExtra("actionButton", actionButton);
        intent.putExtra("cardTemplate", cardTemplate);
    ...
    //add by junwang
    @Override
    public void launchConversationWithH5InfoActivity(Context context, String conversationId, MessageData draft,
                                                     Bundle activityOptions, boolean withCustomTransition,
                                                     int triggerCode, String matcher, String responseUrl, String menuJson,
                                                     String portNumber, int cardMsgTempNo, String cardTitle,
                                                     String regularExpression, String actionButton, String cardTemplate, String chatbotMenu) {
        Assert.isTrue(!withCustomTransition || activityOptions != null);
        //modified by junwang
//        final Intent intent = getConversationActivityIntent(context, conversationId, draft,
//                withCustomTransition);
        LogUtil.i("Junwang", "launchConversationWithH5InfoActivity " +"triggerCode="+triggerCode+", matcher="+matcher+", resposeUrl="+responseUrl);
        final Intent intent = getConversationActivityWithH5MsgInfoIntent(context, conversationId, draft,
                withCustomTransition, triggerCode, matcher, responseUrl, menuJson, portNumber, cardMsgTempNo,
                cardTitle, regularExpression, actionButton, cardTemplate, chatbotMenu);
        context.startActivity(intent, activityOptions);
    }
